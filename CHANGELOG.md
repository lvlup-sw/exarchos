...
### Removed
- **`create-exarchos` interactive installer deleted.** Prior versions vendored serena, context7, and microsoft-learn as extras installed alongside Exarchos by `npx create-exarchos`. Exarchos no longer ships or configures those MCP servers for you; install them yourself if you want them. The primary install paths are now the Claude Code plugin (`/plugin install exarchos@lvlup-sw`) and the standalone binary (`npx @lvlup-sw/exarchos mcp`). Marketplace docs have been updated to drop the stale "Integrations" table rows.
