...
const DISPOSITIONS = Object.freeze([
  ...
  { file: 'servers/exarchos-mcp/src/orchestrate/gate-provider-registry.ts', kind: 'gate-class-definition', count: 3, owner: 'orchestrate/gate-provider-registry', rationale: 'Local phase classes extend the shared taxonomy only at the exhaustive provider registry. THREE definitions, not two: `MechanicalGateClass`, `PhaseGateClass`, and the union alias `SupportedGateClass = MechanicalGateClass | PhaseGateClass`. The alias is not a third AUTHORITY — it introduces no class of its own and changes meaning only when one of its two members changes — but the detector matches `type \\w*GateClass =` textually and cannot tell a union alias from a new taxonomy. Counted here rather than routed around: the alias arrived in 500cc832e (which moved the mechanical vocabulary out of the eval corpus) without this row being updated, so the census was reporting a real, unrecorded third definition. Recording it keeps the both-ways tooth intact — a genuinely new class in this file still trips the count.', category: 'provider-registry' },
