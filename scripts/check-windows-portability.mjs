 ...
 * `servers/exarchos-mcp`, repo-root `scripts/` AND repo-root `src/` — all three
 * emit to a `dist/` or run on a developer's machine, and the default used to be
 * the MCP package alone, which is why a literal `spawnSync('npx', …)` sat in
 * `scripts/lint-envelopes.mjs` unseen: the gate was green about a tree it never
 * opened.
 *
...
const DEFAULT_ROOTS = [
  path.join(REPO_ROOT, 'servers', 'exarchos-mcp'),
 ...
 */
const DEFAULT_SPAWN_HELPER = path.join(
  REPO_ROOT, 'servers', 'exarchos-mcp', 'src', 'utils', 'process.ts',
);
