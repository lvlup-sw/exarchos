...
} from './check-enforcer-wiring.mjs';
...
describe('enforcer-wiring gate — trap-class fixtures (each MUST fail)', () => {
  ...
  it('EnforcerWiring_ExitCodeSwallowedByOrTrue_Fails', () => {
    ...
    t.workflows['.github/workflows/ci.yml'] = t.workflows['.github/workflows/ci.yml'].replace(
      '      - run: npm run guard\n',
...
describe('enforcer-wiring gate — completeness ratchet', () => {
  ...
  it('EnforcerWiring_RetiredEntryStillWired_Fails', () => {
    ...
    const t = baseline();
    t.manifest.primaries[0].disposition = 'retired';
    t.manifest.primaries[0].rationale = 'claims retired but still wired in ci.yml';
...
describe('enforcer-wiring gate — exit-code analysis (the transitive core)', () => {
  ...
  it('ReachPrimariesFromCommand_TransitivelyExpandsNpmChains', () => {
    ...
    expect(reached.has('scripts/check-deep.mjs')).toBe(true);
    expect(reached.get('scripts/check-deep.mjs').failable).toBe(false);
  });
