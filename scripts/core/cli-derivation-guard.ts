...
/** Repository root — `<repo>/scripts/core` → `<repo>`. */
...
export const ALLOWLIST_PATH = 'scripts/core/cli-derivation-allowlist.json';
    ...
    if (!reference.includes('/')) {
      problems.push({
        ...
        detail:
          ...
          '`scripts/core/<file>`) so the reference can be verified against ' +
...
// `scripts/core/cli-derivation-ratchet-guard.ts`.
//
...
//     entrypoint (`scripts/core/cli-derivation-ratchet-guard.ts`),
  ...
  if (!horizonOk) {
    findings.push({
      ...
      message:
        ...
        'scripts/core/cli-derivation-seed-pin.ts is the one deadline every ' +
    ...
    if (horizonOk && entry.expires > horizon) {
      ...
      findings.push({
        ...
        message:
          ...
          'scripts/core/cli-derivation-seed-pin.ts as a deliberate, isolated ' +
...
// own entrypoint at `scripts/core/cli-derivation-ratchet-guard.ts`,
// which is wired blocking and unfiltered. This mirrors DR-4's split between the
// census library and `scripts/core/output-schema-ratchet-guard.ts`.
//
