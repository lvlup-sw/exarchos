...
**Design.** Add a second boundary leg under the lint gate in `static-analysis.ts` — a custom AST rule flagging the results of `JSON.parse` / `response.json()` / `req.body` / `fs.read*` that are **not immediately consumed by a registered parse function**. The parse boundary is a resolved convention (`parsers: ['src/parse/**']`), resolved like a toolchain entry so it is per-repo, not baked. The rule is **two-part** and must encode both halves or the guarantee is illusory: (a) a single parse boundary, **and** (b) no out-of-band `as Brand` / `as any` downstream — because Zod `.brand()` is compile-time-only and one stray cast defeats the whole scheme. For non-TS runtimes the leg **degrades to a Semgrep/CodeQL taint query** (guarantee agnostic, implementation per-runtime — INV-4 parity), documented in the module JSDoc; absent any taint backend the leg SKIP/advisory-degrades exactly like Layer A. Couple it to the SIV-1 boundary axis so it routes on boundary-touching tasks only (R6 cheap-mix), not everywhere.

**Files:** `orchestrate/static-analysis.ts` (the Layer-B leg), a `parse/` registry convention, `testing-strategy-guide.md` (R6 coupling). **Acceptance:** flags a raw `JSON.parse` into core not crossing a registered parser; passes when it does; flags a downstream `as Brand`; documented non-TS degrade path; advisory-skips with no backend.
