...
3. ~~**Design completeness script + handler**~~ — `scripts/check-design-completeness.sh` + `verbs/pure/design-completeness.ts` (T-02)
4. ~~**Plan coverage handler**~~ — `verbs/gates/plan-coverage.ts` wrapping existing `verify-plan-coverage.sh` (T-03)
...
7. ~~**Post-merge check + handler**~~ — `scripts/check-post-merge.sh` + `verbs/pure/post-merge.ts` (T-05, T-06)
8. ~~**TDD compliance handler**~~ — `verbs/tdd-compliance.ts` wrapping existing `check-tdd-compliance.sh` (T-04)
