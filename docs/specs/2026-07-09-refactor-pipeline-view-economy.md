...
**Identity source (stated honestly).** Production adapters do not populate `DispatchContext.cwd`; per `dispatch/core/dispatch.ts` it defaults to `process.cwd()` of the long-lived server process. For a project-scoped server (the normal plugin/CLI arrangement) that is the repo the server was launched in, and `deriveRepoKey` collapses main checkout and all worktrees to one key, so write-time and read-time identities agree by construction. A future client that does thread a real `ctx.cwd` (e.g. via MCP roots) gets more precise identity through the same seam with no further change. This is the deliberate, documented v1 semantics — not an accident.
