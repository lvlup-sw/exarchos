...
Every pipeline response reports `data.scope` (effective mode) and `data.unscopedTotal` (post-phantom, pre-scope-filter count). Whenever `unscopedTotal > page.total` — scoped-empty **and** mixed steady state alike — `next_actions` carries a scope-all escape-hatch hint including the hidden count. Hidden rows are never silent.
...
- `scope: "all"` → no escape-hatch hint (nothing hidden); normal paging affordance only.
