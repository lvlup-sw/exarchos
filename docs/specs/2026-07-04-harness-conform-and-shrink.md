...
- `src/install-skills.ts`, `orchestrate/onboard/install.ts`, `migrations/` legacy-hash manifest + generator — layout, provenance, migration
- `servers/exarchos-mcp/src/onramp/managed-block.ts` (new) + fence-equality guard
- `orchestrate/init/writers/*.ts`, `orchestrate/onboard/*`, `core/onboarding/reconcile.ts`, `orchestrate/doctor/checks/` (+ parity baseline) — block writes, retired-hooks check, ordered uninstall
...
**Files:** `servers/exarchos-mcp/src/orchestrate/onboard/hooks.ts`, `servers/exarchos-mcp/src/orchestrate/doctor/checks/` (new check), `servers/exarchos-mcp/src/core/onboarding/reconcile.ts`, `servers/exarchos-mcp/src/orchestrate/onboard/onboard.parity.test.ts`
