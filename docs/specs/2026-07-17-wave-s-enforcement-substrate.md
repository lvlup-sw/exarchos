...
- **The measured lane is pinned — and the eval lane is preserved, not disarmed:** `test-mcp`'s executed suite varies with `RUN_EVALS` (derived from `changes.outputs.prompts`, `ci.yml:139`), which would confound both the baseline and every comparison. The coverage step runs with `RUN_EVALS` explicitly unset, and the baseline records the lane configuration; the ratchet compares like-for-like only. Because the replaced plain-suite step is today the ONLY execution surface of the `RUN_EVALS`-gated eval tests (`evals/harness.test.ts`, `graders/llm-rubric.test.ts`), task 007 MUST add a **separate conditional eval step** — `if: needs.changes.outputs.prompts == 'true'`, running the eval test files with `RUN_EVALS=1`, outside the coverage instrument — with an acceptance criterion that the eval lane fires on prompts-touching PRs exactly as before. Silently dropping it would be this spec's own condemned defect class.
...
- **Runner substrate:** `@stryker-mutator/core` + the vitest runner land as **pinned devDependencies** of `servers/exarchos-mcp`, with a checked-in stryker config, plus **`servers/exarchos-mcp/scripts/stryker-adapter.mjs`** — the command the `.exarchos.yml` `mutation:` entry resolves to (`node servers/exarchos-mcp/scripts/stryker-adapter.mjs`, runnable from repo-root cwd; no `npx` anywhere). The adapter: accepts the handler-appended `--since=<base>` and translates it to StrykerJS scoping (changed, still-existing, mutatable `servers/exarchos-mcp/src/**` source files from `git diff --name-only <base>...HEAD` — deletions and test files excluded — → `--mutate` globs); executes the **local pinned binary** (`servers/exarchos-mcp/node_modules/.bin/stryker`) with cwd `servers/exarchos-mcp`; and on completion prints the JSON **report file** (`reports/mutation/mutation.json`) to stdout — the handler's parser contract. A diff with an **empty mutatable surface** prints an empty-valid report (`{schemaVersion, files:{}}`) and exits 0 — parseable, never a degrade (an adapter exit code is structurally unobservable through `defaultRunMutation`, which folds any no-report exit into a degrade). Correspondingly, the handler (task 001) treats a parsed report with `total === 0` as a **trivial pass** with an explicit marker (nothing to mutate is vacuously adequate — not a score-0 failure, not a degrade), which also protects the review path from false blocks on server-untouching features. The `toolchains.ts` node-row comment that mislabels `--since` as a StrykerJS flag is corrected to document the adapter contract. An **end-to-end composed-path smoke test** exercises resolver → handler default runner → adapter → stryker → parser on a tiny fixture scope, within a documented runtime budget — running as its **own Linux-only `test-mcp` step outside both the coverage instrument and the default suite glob** (so it is never conscripted onto the blocking Windows lane's known spawn-flake class, and never inflates the coverage-measured lane).
...
- `outcome-tests` and BOTH path filters are byte-identical to main (per DR-3: no filter changes anywhere in this spec).
...
**Expected tests:** `Passed_DiffScopeKilledPlusNoCoverageMix_Fails`, `Passed_AllCoveredAtThreshold_PassesUnchanged`, `Passed_NoCoverageWithinExplicitBudget_Passes`, `Passed_EmptyMutatableSurface_TrivialPassWithMarker`, `FailureMessage_NoCoverageMutants_AttributesFileAndLine`, `Fold_LegacyMutationEventWithoutNoCoverage_FoldsIdentically`, `GuardCheckFour_NoCoverageExceedsBudget_BlocksUnderEnforcement`, `GuardCheckFour_AllCovered_PassesUnchanged`, `Injection_MaxNoCoverageConfigured_ReachesGuard`
**Verification:** high rung: scoped tests + adequacy probe + the projection/guard integration suites; carrier fields and `mutationScore` semantics byte-identical for existing consumers (INV-5b); legacy fixture log folds byte-identical (INV-1); both facades see the identical change (INV-2 — the pass-computation lives only in the shared handler; the `_maxNoCoverage` injector edit is config-resolution plumbing, not a facade fork); the no-toolchain skip-pass path still folds as skip-pass (review→synthesize dead-lock guard unchanged for runner-less repos)
**Dependencies:** None
**Parallelizable:** Yes
...
**Verification:** medium rung: self-test both directions + both fail-closed paths; the skip/failure taxonomy matches DR-7's enumeration exactly; enforcer-wiring audit shows only findings of the expected `[unlisted-primary]` class for this spec's not-yet-wired scripts (up to three on the integrated tree at this point — entries land with the wiring in 007)
...
### Task 006: Close the skipped-as-passed hole and the implementation-surface hole (skip-guards, measured dispositions, no filter changes)
...
**Files:**
- `.github/workflows/ci.yml` (`test-mcp`: the plain suite step becomes the coverage-instrumented run — `npm run test:coverage` with `RUN_EVALS` explicitly unset — then `check-coverage-ratchet.mjs --observe`, then upload `coverage-summary.json` as an artifact; plus a **conditional eval step** — `if: needs.changes.outputs.prompts == 'true'`, running the RUN_EVALS-gated eval test files with `RUN_EVALS=1` outside the coverage instrument, preserving the eval lane the replaced step carried; plus the mutation step — `check-mutation-gate.mjs --observe`, PR-events only with explicit base fetch; plus the Linux-only composed-path smoke-test step (012's, outside the coverage run); grep-gates zero-dep prefix gains the type-debt step; grep-gates tsx tail gains the vocabulary-lint step and the ci-topology-test step (`npx --no-install vitest run scripts/ci-topology.test.ts`); grep-gates gains `.test.sh` re-assert steps for check-type-debt, check-coverage-ratchet, and check-mutation-gate (task-015 pattern); each step carries its DR-1 host-class comment)
...
**Verification:** medium rung: on this task's own PR push, `test-mcp` runs the instrumented suite + observe-mode mutation green and grep-gates runs type-debt + vocabulary-lint + topology test + re-asserts green; the conditional eval step's `if:` condition exactly reproduces the old job-level `RUN_EVALS` derivation (eval lane fires on prompts-touching PRs as before — DR-5 acceptance); the enforcer-wiring audit is FULLY green (zero pending states; the observe-window entries carry `advisory` disposition with rationale, flipped to `gating` by 009); `npm run lint:invariants` run once locally before landing (current tree is believed clean — verify, don't assume); both filters and `outcome-tests` byte-identical to main; coverage artifact appears on the run
...
- `scripts/enforcer-wiring-manifest.json` (flip the two observe-window entries `advisory` → `gating` in the same change — the manifest's honesty contract tracks the actual blocking state)
...
**Dependencies:** 001, 009 (the "blocking CI backstop" framing must not precede the actual flip — claiming a backstop while the steps are observe-mode is the #1704 defect verbatim)
**Parallelizable:** Yes (after 001, 009)
**testingStrategy:** propertyTests: false, benchmarks: false
...
**Implements:** DR-10
**Files:**
- `docs/specs/2026-07-17-wave-s-enforcement-substrate-baseline.md` (new — the evidence record: every new gate's self-test results in both directions, the fail-closed matrix, the filter byte-identity proof (delta: none), allowlist state at merge, the DR-4 measurement, coverage baseline provenance, the eval-lane preservation proof (the conditional eval step fires on a prompts-touching diff), manifest completeness — every new check script has its entry and its grep-gates re-assert step)
...
- **Wave B (parallel, after their dependencies):** 004 (after 001 + 012); 010 lands last-but-one (after 001 + 009 — its framing claims the flipped state)
