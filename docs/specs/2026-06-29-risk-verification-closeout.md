...
- **Plan-review bound (DR-1).** `state-machine.ts`: add `isRevision?: boolean` to the transition type + the `if (transition.isRevision)` emission of a `plan-revision` event (mirror lines 815-819); add `countPlanRevisions`. `views/workflow-state-projection.ts`: fold `plan-revision` → `state.planReview.revisionCount`. `hsm-definitions.ts createFeatureHSM`: set `isRevision:true` on `plan-review → plan`, order `plan-review → blocked` first. `config/resolve.ts`: add `workflow.maxPlanRevisions` (default 1) beside `maxFixCycles`; project the resolved cap into `state.planReview.maxRevisions` for the pure guard `revisionsExhausted` to read. `commands/plan.md`: re-plumb the loop through `transition`.
...
- `config/resolve.ts` — `review.mutationEnforcement` key; `workflow.maxPlanRevisions` (default 1), projected into `state.planReview.maxRevisions`.
...
**Risk Tier:** high · **Boundary Touching:** true · **Implements:** DR-1
**Files:** `workflow/hsm-definitions.ts`, `workflow/guards.ts`, `config/resolve.ts`, `views/workflow-state-projection.ts` (project `maxRevisions`), co-located `*.test.ts`
**Verification:** high — guard/transition tests proving `blocked` wins at cap, `workflow.maxPlanRevisions` default 1 + `.exarchos.yml` override honored, resolved cap projected into state; integration across the HSM.
