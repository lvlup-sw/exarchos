...
| Layer enforcement, second authority | `.dependency-cruiser.cjs` `no-domain-core-to-io-adapters` is `severity: 'error'`, scoped `^servers/exarchos-mcp/src/(event-store\|workflow)/` → `adapters/`, executed by `runBoundaryLint` (`orchestrate/pure/static-analysis.ts:216`) |
