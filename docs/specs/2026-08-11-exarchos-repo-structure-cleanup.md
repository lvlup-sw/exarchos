...
>
> **Found DURING execution — four more the pre-flight measurement missed.**
>
> 6. **Premise 1's rule is right for directories and wrong for the file itself.** "Preserve what the expression resolved to" holds only while the SUBJECT stays put. Three subjects do not. A test that reads its own source (`sdk-generation-seam.test.ts:43`) travels with the file, so preserving the old path points at nothing. Fixtures travel in LOCKSTEP — `join(__dirname, '__fixtures__')` needs no edit at all, because the mirror mapping keeps the test→fixture offset identical. And a *bare* anchor used to scan its own directory (`relative(ADAPTERS, …)`, 10 sites in 5 files) has no literal segment to adjust, so it silently comes to mean the new directory. The rule that covers all four cases is **preserve the TARGET, not the path**: leave anchors alone, recompute the literal segments, and pin the declaration outright where there is no literal to recompute. Only `adapter-direction` failed loudly — it carries a companion "the rule has a real subject" assertion. The other four would have gone vacuous, invisible to a per-test diff.
> 7. **`protected-suites.json` DOES exist and is tracked** (`61fa6d938`), so premise 4's "absent entirely" is spent. What was true: the CLI has no host. Two further corrections — `src` must come OUT of `PROTECTED_ROOTS` (it now holds zero keep-class suites, and `PROTECTED_ROOTS_ALL_EXIST` fails a root that contributes none), and `tests/integration` must NOT go in for the same reason. The live host is `npm run protected:guard`; it is deliberately NOT a blocking CI gate yet, because this task's own change-set touches all 53 protected files by moving them. Wiring belongs to 058.
> 8. **Two config path-pins die with the move, and one of them was hiding a broken test.** `vitest.config.ts` excludes the heavy Stryker smoke test by LITERAL path, and `.github/workflows/ci.yml` runs it by literal path. The move made both dead, conscripting the test into the default lane — where it turned out to be red since task 019's fold rewrote `servers/exarchos-mcp/src/` → `src/` and collapsed one assertion into a contradictory duplicate of another. It has been failing in its own CI step ever since, unnoticed.
> 9. **Task 029's "each move task keeps `tests/tsconfig.json` green" is not achievable here.** The 925 movers had never been type-checked anywhere (the root config excludes `**/*.test.ts`), and putting them under that program surfaces **2,801 errors across 482 files** — 789 once `noUncheckedIndexedAccess` is discounted. Debt the move exposed, not caused, and paying it means semantic edits to over half the test tree. `unit/**` and `integration/**` are excluded, which holds the line exactly where it stood before the move, with `TestsTsconfig_ExcludedTiers_OnlyShrink` keeping the exemption from widening.
...
> **Premises corrected during execution (measured 2026-08-13 against `worktree-exarchos-overhaul-staging`).**
>
> 1. **Half of this task was already done, and the other half is bigger than stated.** The "61 co-located tests under the former root `src/`" is **zero** — task 030 took all of them, and the only thing left under `src/` is the deliberate `declaration-seam-violator.fixture.ts`. The "77 under `scripts/`" is **87**: 48 `.test.ts` and 39 `.test.sh`.
> 2. **`tests/unit/` is a DIRECTORY name, not a project name, and the Files field above names the wrong destination.** `tests/unit/**` is collected by the **`core`** project (`vitest.config.ts:130`), which task 030 chose deliberately so the product's own tests keep the `bun:sqlite` alias and the 60s Windows headroom. Routing the 43 script suites there would have handed them a 12x timeout budget and a different pool through an already-existing glob, with nothing to notice — the tier change is invisible to a per-test status diff because every test still passes. They go to **`tests/scripts/`** (the `unit` project, 5s, beside the `tests/evals/` tree that project already collects); the five under `scripts/core/` go to **`tests/core/scripts/`**, which `:124` already collects into `core`. Preserving the split is what "no tier changes" means operationally.
> 3. **"Keep their runner wiring" means 37 literal paths in two workflows.** Every `.test.sh` is invoked as `bash scripts/<name>.test.sh` from `ci.yml`/`release.yml`, and several `.test.ts` are named as vitest path operands. These are repinned from the same move map that moves the files, so a path cannot be updated in one place and missed in another.
> 4. **The bash suites need exactly one edit each, and it is not a path rewrite.** All 39 self-anchor as `$(dirname "${BASH_SOURCE[0]}")` and reach only STAYERS through it — their subject gate in `scripts/`, the repo root one level up. Inserting the new→old segment straight after the `dirname` pins the anchor, and `REPO_ROOT="$SCRIPT_DIR/.."`, `$SCRIPT_DIR/<gate>` and every `cd` downstream keep their original meaning untouched. Their oracle is an exit-code diff, not the per-test JSON: they belong to no vitest project, so the suite diff cannot see them at all (45 repo-wide, 25 passing / 20 failing, unchanged across the move).
> 5. **`selfTestCandidates` mirrors `src/` and knew nothing of `scripts/`.** Four call sites pair a gate with its self-test by CONSTRUCTED path (`guard-inventory.ts:1195,1271,1654,2065`, all `existsSync`). Task 030 added the `src/` → `tests/unit/` arm for exactly this reason; without a `scripts/` arm every gate under `scripts/` silently stops pairing, and an unpaired gate reports as "no self-test" rather than as an error — a whole tree of guards going quiet with nothing red. Fixed, and pinned by `SelfTestPairing_GateUnderScripts_IsFoundInTheRelocatedTestTree`, which asserts the denominator as well as the mapping.
> 6. **Premise 9 of task 030 does NOT carry over: this debt is payable, and the ratchet says pay it.** The movers surfaced 73 type errors across 16 files, not thousands, and `TestsTsconfig_ExcludedTiers_OnlyShrink` exists precisely to make adding a third exemption fail first. Paid: `allowJs`/`checkJs:false` lets the checker infer types from the `.mjs` gates the suites drive (which erased 49 of the 73 — an untyped module also erases the contextual type of every callback passed to it), and the remaining 19 were narrowed. Net **−5 casts** against the near-saturated DR-14 ratchet, because three `as` assertions were what widened the fixtures out of their own signatures.
> 7. **Two guards were found lying, both invisible until a typechecker read the file.** `guard-inventory.test.ts` asserted `expect(liveInventory.artifactsWithSelfTest ?? []).not.toContain(artifact)` — there is no such field, so the expression was `[]` on every iteration and the loop asserted nothing; the `?? []` is what made it silent instead of a crash. And a `GuardHost` fixture carried `pathFiltered: false`, a property that does not exist (the type has `pathFilterKeys`). Neither could fail, in either direction.
> 8. **`scripts/__fixtures__/` splits: one half travels, one half must not.** `ci-topology/` (5 `.yml`) travels — its only referrer is a `__dirname`-relative join in the consumer that moves with it. `comment-hygiene/` (3 files) **stays**: it is named by a production policy glob (`.exarchos/comment-policy.json`), the knip allowlist and `eslint-rules/comment-content.test.js`, all by repo-root-relative string. Moving it would blank that glob silently — the comment-hygiene gate is not enabled yet, so nothing would have gone red. Left in place, every one of those references keeps working with no edit.
> 9. **`lint-envelopes.test.sh` leaves a stray `src/orchestrate/` behind, and it reds a suite that never ran it.** Its `mkdir -p` creates both levels and its cleanup removes only the leaf. That directory stopped existing when `orchestrate/` became `verbs/`, so the leftover reads as an unmapped source directory and fails six assertions in `tests/architecture/layer-map.test.ts`. Cleanup now removes what it created.
> 10. **Relocation into a governed root manufactures debt exactly as relocation out of one discharges it.** The five `scripts/core/` guards land inside `tests/core/`, a DR-30 corpus root — a tree they were never covered by at `scripts/core/`. Admitting them would post six new shape violations that no code change caused. `ScanRoot.excludePrefixes` is the symmetric half of "follow the files", and it is reserved for relocation: a file that genuinely belongs to a root still has to declare its authorities or take a registered, expiring gap.
...
**Testing Strategy:** exampleTests true, propertyTests false, benchmarks false, characterizationRequired true.
**Files:**
- `tests/scripts/`, `tests/core/scripts/`
**Tests:**
- `TestInventory_AfterRemainingColocatedMove_NoIdIsLost`
- `SelfTestPairing_GateUnderScripts_IsFoundInTheRelocatedTestTree`
**Verification:** Move the 87 co-located tests under `scripts/` (48 `.test.ts` + 39 `.test.sh`) into `tests/scripts/`, and the five under `scripts/core/` into `tests/core/scripts/`, so each keeps the vitest project that collected it. The `*.test.sh` bash suites keep their runner wiring: their 37 literal invocations in `ci.yml`/`release.yml` are repinned from the same move map. The `src/` half of this task was already discharged by 030.
**Dependencies:** 030
**Parallelizable:** No
...
> **Premises corrected during execution (measured 2026-08-13).**
>
> 1. **The root `evals/` is 33 files, not 31**, and it is DATA — suite manifests and `.jsonl` datasets, zero tests. The measurement code that scores it left `src/` in task 019 and lives at `tools/evals/`. `docs/ARCHITECTURE.md`'s "`evals/`, `benchmarks/`, `bench/` → `tools/evals/`" line reads like a contradiction of this task's destination and is not one: it maps subtrees **of `src/`**, and the two `evals` are different things that happen to share a name.
> 2. **`benchmark-gate.yml` needed no retarget.** It reads `src/projections/telemetry/benchmarks/baselines.json` and runs `npm run bench`, whose `benchmark.include` names `src/**` and `tools/evals/bench/**`. Neither touches the moving trees. The root `benchmarks/baselines.json` is named by exactly two files, and both describe it as **superseded**. Only `eval-gate.yml` (its path filter and both `EVALS_DIR` values) and `ci.yml`'s `prompts` filter were live.
> 3. **The whole of `docs/evals/` moves, not just the graders.** 8 write-ups, `data/` (11 evidence files) and 95 grader files. DR-7 re-homes "graders **and datasets**" and reduces `docs/` to the VitePress skeleton plus three named entries, so leaving the write-ups behind would contradict the requirement this task implements.
> 4. **A slash-anchored reference census cannot see a segmented path.** `path.join(REPO_ROOT, 'evals', 'reliability')` carries no `evals/` substring, so the census that found all 75 other referrers reported it clean; four tests went red and only the per-test status diff named them. A move needs a second sweep for the quoted bare segment. That sweep found 26 more occurrences, 25 of which are directory-NAME predicates matched at any depth and correctly unaffected.
> 5. **Task 031's path rewrite had edited a test NAME inside the oracle.** `tools/audit/test-inventory-baseline.json` recorded `passes the full tests/scripts/get-exarchos.test.sh suite` while the source still said `scripts/…` — a blanket literal rewrite over the baseline JSON caught a path that happened to sit inside a test title, desyncing the oracle from its subject. The source is now the one that changed, so the two agree. The committed baseline was also three guards stale (031/032 added guards without regenerating it).
> 6. **Two populations of the moved tree are not code, and excluding them is a category statement rather than debt.** The 69 captured run artifacts under `runs/` are verbatim records of what a model wrote, scored as-is — editing one to satisfy a compiler falsifies the measurement. The three `tasks/*/oracle.ts` hidden graders import `./impl.ts`, the candidate implementation that is materialized per run and deliberately absent from the tree, and run under `tsx` from that directory. `TestsTsconfig_ExcludedTiers_OnlyShrink` now holds the debt list and the not-code list separately so one cannot be smuggled in wearing the other's label.
> 7. **The `docs/**/runs/**` exclusion was one move away from going vacuous, in the direction that matters.** These artifacts use a `process.exit` module-load harness; reaching a vitest worker they take the worker down rather than fail. While they sat under `docs/` no test glob reached them anyway, so the exclusion was load-bearing only in principle. Landing them under `tests/` makes it load-bearing in fact, at the same moment a `docs/`-anchored glob stops matching. Re-anchored on `**/runs/**` and covered by a guard that asks the resolved runner what it collects, not what the config says.
> 8. **The typecheck debt is payable again** (127 errors, 59 files → 0), and two more latent defects surfaced. `computeVerdict` opened with a CE-priority branch that could never fire — `SampleVerdict` has no `ce` member, and the test named `computeVerdict_AnyCe_ReturnsCe` already asserted `'fail'` and explained why in a comment. And `RunnerDeps.loadArm` advertised `(armsDir: string, armId: string)` while the only implementation injected into it accepts `ArmId`, so the seam promised a contract nothing could honour. Twelve `EventEmitter → Record` casts in the executor tests collapsed to one at the factory, a net −11 against the DR-14 ratchet.
> 9. **`resolveEvalsDir()` has never resolved the data directory** (`tools/evals/evals/run-evals-cli.ts`). It walks upward for a directory named `evals` and finds its own — `tools/evals/evals` — on the first parent. That has been true since task 019 renamed its home and is unchanged by this move (CI sets `EVALS_DIR` explicitly, which is why nothing noticed). Recorded, not fixed: it is a semantic change and Phase 1 carries none.
...
> **Measured and attempted 2026-08-13; reverted before commit. Read this before re-running.**
>
> The move itself is settled — destination map, collision proof and codemod all exist and run clean. What defeated the attempt is the *reference tail*: four distinct classes, three of which no path-shaped search can see. The move was applied, typechecked green across all four programs, and still left 101 vitest failures and 19 shell-suite regressions. It was reverted rather than committed red.
>
> 1. **The tree is 103 files, not 151.** `scripts/` is 90 (tasks 003 and 031 already drained it), `eslint-rules/` 8, `renovate-config/` 4, `migrations/` 1.
> 2. **Destination map (no collisions).** `scripts/audit/` → `tools/audit/`; `scripts/{core,lib,__fixtures__,__shims__,test-fixtures,tsconfig-strictness}/` → `tools/audit/<same>/`; the 15 build/publish/sync scripts → `tools/release/`; the remaining 40 flat gates → `tools/audit/gates/`; the other three roots → `tools/<same>/`. That yields exactly the five directories the Files field names, `gates/` being a subdirectory rather than a sixth top-level entry.
> 3. **Four reference classes, and only the first is visible to a text sweep.**
>    (a) 169 files carrying slash-anchored paths.
>    (b) `$GITHUB_WORKSPACE/scripts/…` in CI — 20 sites. A lookbehind added to stop `tests/scripts/lib/…` being rewritten into the mover's destination also blocks these, because both are preceded by `/`.
>    (c) **Bare parent walks** — 23 sites of `resolve(SCRIPT_DIR, '..')`. From `scripts/` that reached the repo root; from `tools/audit/gates/` it lands inside `tools/audit`. There is no path literal to match, only a depth that changed.
>    (d) **Segmented joins** — 40 sites of `join(root, 'scripts', 'x.mjs')`. The same class that cost task 033 four red tests, at ten times the count. A census must sweep for the quoted bare segment, not just the slash form.
>    (e) 27 shell harnesses computing `SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../scripts" && pwd)"`, plus 12 more that use `$SCRIPT_DIR/..` as a stepping stone to the repo root and need their destination decided individually.
> 4. **Two false positives a blanket regex would have corrupted.** `./types.js` in `codegen-runtimes.ts` sits inside the TEMPLATE LITERAL of the file it generates, so it is relative to `src/install/runtimes/`, not to the generator; rewriting it breaks the generated `embedded.ts`. And `$MOCK_REPO/scripts/validate-installation.sh` is the layout of a throwaway repo the installation test *builds*, not a reference to this tree. Both must stay as written.
> 5. **`tsconfig.scripts.json` is the trap this task sets for itself.** Its `include` is `scripts/**/*.ts`, it is NOT in `npm run typecheck` (which runs root + conformance + tests), and it is executed only by a dedicated `ci.yml` step. Move the tree and it silently resolves nothing. `tsconfig-scripts-coverage.test.ts` is what catches it, and that guard needs its `tree: 'scripts'` widened to the two new roots — a single prefix cannot express a tree that split in two.
> 6. **The published contract is the one genuinely open decision, and the spec does not cover it.** `manifest.json` ships `{id: scripts, source: scripts, target: scripts}` as an installed component and `package.json files[]` publishes `scripts` — neither is mentioned in this task. `target` must stay `scripts` (it is the user-facing `~/.claude` layout, and `V1_SYMLINK_NAMES` in `src/install/operations/migration.ts` knows that name). `source` and `files[]` were set to `tools/audit/gates` on the attempt, which SHRINKS the shipped set — previously all of `scripts/` shipped. Pointing either at `tools` instead is not an option: that would publish `tools/audit`'s 2.4 MB inventory baseline plus `conformance/` and `evals/`. **This needs an explicit decision about what the installed `scripts/` component is for, validated by the `acceptance` project's fresh-install suite.**
> 7. **`scripts/get-exarchos.{sh,ps1}` needs no published-location work.** The URL is `https://lvlup-sw.github.io/exarchos/get-exarchos.sh`, served from `documentation/public/`, which `docs.yml` COPIES from the repo path at build time. Only that copy step and its path filter move; the public URL is unchanged.
> 8. **A duplicate pair is waiting under the destination.** `tools/skill-validators/` already holds `validate-all-skills.sh` (67 lines) and `validate-frontmatter.sh` (237), while `scripts/` holds divergent files of the same names (140 and 181). They have different callers — the `tests/scripts/` suite drives the `scripts/` copies, `tools/skill-validators/validate-frontmatter.test.sh` drives its own — and `tools/skill-validators/validate-all-skills.sh` has no caller at all. Destinations differ so nothing collides, but the duplication is real and belongs to task 053.
> 9. **`knip.json` is already stale in ways this task will be blamed for.** `scripts/**/*.test.ts`, `test/**/*.test.ts`, `test/**/*.ts` and `test/**/*.mjs` have matched nothing since tasks 031 and 032. Only `scripts/**/*.ts` is this task's to retarget; the rest is task 042's glob-liveness sweep.
> 10. **The guard-liveness baseline has 11 files of headroom left.** `GuardLiveness_Baseline_IsCurrentWithTheTree` fails when the tracked-file count drifts 50 from the task 003 capture; it is at **39** (3,355 → 3,394). Tasks 033–035 spent it. The next task to add a dozen files trips it, and the remedy is a deliberate re-capture rather than a quiet bump — a baseline re-taken as a side effect of something else is the "comparison against fiction" its own comment warns about. Task 036 will blow past it outright: 103 files change path, and the counter measures the whole tree.
>
> **Premise 6 RESOLVED, and landed ahead of the move.** The question was what the installed `scripts/` component is for. It is for nothing: `grep -rn "components.core" src/` returns zero — no installer code reads the array, so `target: scripts` never materialized, and `~/.claude/scripts` is absent on a machine running 2.12.0-preview.3. `plugin.json` declares no `scripts` key either. The only runtime tie is `V1_SYMLINK_NAMES`, which **deletes** a v1 leftover and is kept. So the component and the `files[]` entry were removed rather than repointed — repointing `source` anywhere would have published a fiction, and `tools/audit/gates` in particular would have shrunk the shipped set 88 → 40 with nothing to notice.
>
> Three consequences worth carrying into the move. **`files[]`'s negations went dead with it**: `scripts/` was the last shipped root holding fixtures or test files, so `!**/test-fixtures` and `!**/*.test.{sh,ts}` now exclude nothing — `npm pack` is byte-identical with and without all three, and they are retired on the same rule that retired `!**/trigger-tests`. Keeping test-only paths out of the tarball is asserted against the artifact instead, in installer-verify's pack test, which is strictly stronger: it catches `get-exarchos.ps1.test.ps1`, which the `.sh`/`.ts` negations never could. **The fresh-install suite cannot police this** — it asserts each component `source` exists with >0 files, so a bogus-but-existing source passes; it stayed green with a deliberately planted junk component. Premise 6 named it as the validator, and it is necessary but not sufficient. **`guard-liveness-baseline.json` now carries one more stale surface** (`package.files:scripts`), joining the five `package.files:` entries already stale since the DR-4 split; it is a frozen capture and wants the same deliberate re-capture premise 10 describes, not a quiet edit.
>
> Lines 216, 416 and 1202 above describe the superseded contract — they are task 026's record of what was true when it ran, left as written.
