...
- **Falsifier (the assertion DR-0 lacked):** seed a module importing an SDK package directly; the seam rule must reject it. A seam with no current failing subject has not been shown to work — and on introduction there are **<!-- measured: sdk-import-sites -->40<!-- /measured --> such subjects across <!-- measured: sdk-import-directories -->13<!-- /measured --> directories**, so the denominator is non-empty by measurement, not by assertion.
