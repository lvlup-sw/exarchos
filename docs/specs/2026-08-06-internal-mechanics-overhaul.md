...
| **Kill fixture** | The **hand-written literals in the composition root** — `doctor`, `version`, `feedback`, `schema`, `topology`, `emissions`, `mcp`, `onboard`, `init`, `install-skills`, now <!-- measured: cli-handwritten-literals -->10<!-- /measured --> of them. The guard reported all ELEVEN on introduction (the eleventh, `merge-orchestrate`, was deleted by task 076). Line numbers are deliberately not transcribed here: they drift on every edit to `cli.ts` and the parse resolves them anyway. |
...
| **Exceptions** | The <!-- measured: cli-handwritten-literals -->10<!-- /measured --> literals enter an allowlist keyed by command name, each with an owner and an **ISO expiry date** (not a "wave-scoped" label, which is not mechanically evaluable). Entries may only be removed. **`merge-orchestrate` is excluded from the allowlist** — it was the kill fixture and had to remain rejected; rev 1 allowlisted it, neutralizing the very rejection DR-5 requires. **Task 076 deleted its hand-written command, as this row required — not exempted it**, so it is no longer among the literals above. The exclusion remains declared policy (`KILL_FIXTURE_COMMANDS`), and the guard's proof that it would still be rejected is re-seeded against a synthetic parse. |
...
- The <!-- measured: cli-handwritten-literals -->10<!-- /measured --> hand-written top-level verbs enter an allowlist with per-entry owner and wave-scoped expiry; **the allowlist may only shrink**, enforced by ratchet.
...
7. **What exactly should `sdk-import-sites` count?** *(opened rev 4.3 by task 024.)* The claim is bound to a precise derivation — *files under `servers/exarchos-mcp/src` referencing the v1 specifier, **test files included*** — and DR-27 correctly caught it drifting 38 → 40 as this wave added tests that mention the specifier as fixture text. But the **name is ambiguous even though the derivation is pinned**: task 024, measuring non-test files through the seam's own classifier, got 27 and read the difference as a disagreement. Two honest derivations, two different populations, one name.
   ...
   - **CLOSED by task 053 (rev 4.11): the backlog is 0 / 0 / 0.** All 42 sites across 22 files migrated onto `contract/sdk/seam.ts`. Unlike every prior movement of these numbers, **this one is the tree changing, not the instrument** — the same instrument, at the same scan root, reports 42 → 0.
...
> **Corrected at implementation, rev 4.9; re-derived at rev 4.18.** This line previously named EIGHT verbs and INCLUDED `merge-orchestrate`. Both were wrong when re-derived against the tree: the parse reported ELEVEN literals (`feedback`, `schema` and `topology` were omitted), and `merge-orchestrate` is the kill fixture that G1's own Exceptions row forbids allowlisting — the shipped guard REFUSES a policy file that names it, so following this line would have produced a policy file the mechanism rejects. The tracked population was therefore <!-- measured: cli-allowlisted-literals -->10<!-- /measured -->. Task 076 then deleted the kill fixture's hand-written call, so the literal population fell to <!-- measured: cli-handwritten-literals -->10<!-- /measured --> and **the two counts now coincide** — every remaining literal is tracked debt, which is the state that let G1 be wired blocking.
>
...
**Files:** `servers/exarchos-mcp/src/contract/sdk/seam.ts` (new), `servers/exarchos-mcp/src/contract/sdk/brand.ts` (new)
...
3. **The v2 half of the seam stays unreferenced, and that is task 049's, not a defect here.** v2 has zero import sites tree-wide; 11 of the 23 knip findings on `contract/sdk/seam.ts` are v2 surface or uninhabited holes and cannot be discharged by a v1 migration. Reported per-symbol, not allowlisted.
...
**Landed (rev 4.18).** The hand-written promotion is deleted; `merge_orchestrate` now carries `cli: { topLevel: 'merge-orchestrate' }` and is hoisted by the DR-7 loop through `registerActionCommand`. `cli-derivation-guard` exits **0** (13 sites, 0 violations), is wired **direct and blocking** on the unfiltered `grep-gates` deps tail, and its `GUARD_EXEMPTIONS` entry is deleted. **Wave 1's unreachable set is now EMPTY** — this was its last member.

Three findings, reported rather than worked around:

1. **Criterion 2 is discharged as NOT APPLICABLE, and the premise behind it was wrong.** The task assumed removing a promoted top-level verb is user-visible and therefore owes an `init`-style rename stub — "a judgement call that does not belong inside a ratchet task." But DR-7 had *already* shipped `CliActionHints.topLevel`, whose hoist loop registers a top-level command from a registry declaration. Moving the promotion onto that hint deletes the literal while leaving `exarchos merge-orchestrate …` byte-identical for operators: same flags (both generate from the same schema), same handler, same exit codes. **Nothing was renamed — only the place the name is declared moved** — so there is no stub to write and no release note to publish. No live doc names the verb (only an archived design), so nothing needed updating. Recording this because "the criterion did not apply" and "the criterion was skipped" look identical in a green build.
2. **A rename stub was not merely unnecessary — it was impossible.** Had the verb genuinely needed one, the stub would itself be a hand-written `.command('merge-orchestrate')` literal, and `readPolicy` refuses any policy file naming a kill fixture. Criteria 1 and 2 as written are jointly unsatisfiable for this verb. The `cli.topLevel` route is the only path that satisfies both.
3. **The exit-code ladder changed carrier, not behaviour.** The deleted block reimplemented a three-arm map by hand; the hoisted path uses `resolveExitCode`, whose own note records that it is a strict superset for every handler-reachable code. `cli-merge-orchestrate.test.ts` (5 tests, all four exit codes) passes unchanged, which is the parity evidence.

Two proofs were re-seeded rather than deleted with their subjects — the guarantees would otherwise have lapsed at the moment they became load-bearing: task 021's kill fixture (splice the deleted command back into the real composition root, re-run the same guard) and `GuardInventory_SelfTestHostedGateWithNoDirectExecution_ReadsAsUnreachable` (synthetic, since the live subject was this very guard). A third, `GuardInventory_IndirectionDidNotMakeEverythingReachable`, lost its falsifier when the unreachable set emptied and now seeds an unwired guard into the live inventory to show "empty" still means "checked".
