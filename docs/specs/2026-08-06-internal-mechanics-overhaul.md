...
| **Response shape** | `outputSchema` (118); `Envelope<T>` type; the runtime payload | `outputSchema` nominally | **No** — <!-- measured: output-schema-vacuous -->112<!-- /measured --> vacuous | Authority exists but asserts nothing. → **G2** |
...
| **SDK generation** *(new in rev 4; corrected rev 4.3)* | `@modelcontextprotocol/sdk` (v1) **imported**, <!-- measured: sdk-import-sites -->23<!-- /measured --> importing files across <!-- measured: sdk-import-directories -->9<!-- /measured --> directories (<!-- measured: sdk-import-production-files -->10<!-- /measured --> of them non-test) — *was 40 / 13 / 16 until task 061 replaced the raw-text scanner with a parse; the correction is in the instrument, not the tree*; `@modelcontextprotocol/{core,server}` (v2) **installed but with ZERO production import sites** | *contested* | **No** | **2 authorities — but not in the way rev 4 stated.** Rev 3 omitted this row entirely while DR-0 depended on it; rev 4 added it claiming both generations are "imported directly", which task 024 measured as **false**. The contest is between an **imported** v1 and an **installed-but-unused** v2, and structural typing means nothing — not `tsc`, not a guard — separates them once v2 *is* used. → **G6 / DR-26** |
...
| **Kill fixture** | The <!-- measured: output-schema-vacuous -->112<!-- /measured --> current vacuous declarations — the ratchet's initial value, and its proof of a live subject. |
...
- The **<!-- measured: output-schema-vacuous -->112<!-- /measured --> existing vacuous declarations** become an explicit **shrink-only allowlist**, each with an owner and an ISO expiry. Entries may only be removed. The list is seeded **from the census output, derived at introduction — never written as a literal.**
...
- **Falsifier (the assertion DR-0 lacked):** seed a module importing an SDK package directly; the seam rule must reject it. A seam with no current failing subject has not been shown to work — and on introduction there are **<!-- measured: sdk-import-sites -->23<!-- /measured --> such subjects across <!-- measured: sdk-import-directories -->9<!-- /measured --> directories**, so the denominator is non-empty by measurement, not by assertion. *(Was 40 across 13 until task 061 replaced the raw-text scanner with a parse; the tree did not change, the instrument did.)*
...
**Detail:** Enumerate declarations; classify `EnvelopeSchema(z.unknown())` as vacuous. Must report **<!-- measured: output-schema-vacuous -->112<!-- /measured -->** on introduction — its proof of a live subject.
...
- [ ] G2 seeded at <!-- measured: output-schema-vacuous -->112<!-- /measured --> and shrink-only; new vacuous action fails CI
