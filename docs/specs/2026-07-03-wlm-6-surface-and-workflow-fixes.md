...
- **(2) Bound the plan-review loop at its own unskippable seam — `prepare_review scope:plan` — mirroring how the delegate loop self-bounds at its seam; no unification.** The two loops are irreducibly different (different verdict-application paths, different counters; the plan loop has no scoring handler), so there is no shared primitive to build — the delegate loop already self-bounds at `check_review_verdict` and needs no change. Give the plan-review loop the bound it *lacks*, at the one server action it *cannot* skip to re-review: make `buildPlanReviewProvisioning` (`prepare-review.ts:176-210`) **stateful** — on each dispatch it emits a counted `plan-review-dispatched` event and **refuses to provision another review once the count reaches `_maxPlanRevisions`**, returning a park-at-`blocked` affordance (INV-12). The count now rides the seam the agent *must* call, so the skippable-edge bypass is closed by construction. The seam emits the counted `plan-review-dispatched` event **only on a re-dispatch** (when prior dispatch events already exist for the feature) — the initial review is revision 0 — so `planReview.revisionCount` folds `+1` per event through the *existing* projection unchanged, and `revisionCount` = re-dispatches = revisions (a re-dispatch happens only after a gaps verdict; a "survives" verdict yields no re-dispatch, consuming zero). The `isRevision`-edge emission at `state-machine.ts:872-881` is retired as a counter source **for the standard `plan-review → plan` edge only** — that same block also fires for the `overhaul-plan-review → overhaul-plan` edge (`hsm-definitions.ts:552-561`), whose loop is a **human checkpoint** (`playbooks.ts:1005-1023`) that never dispatches through `prepare_review`, so the overhaul loop **keeps its edge counter untouched** (it has no in-place bypass — a human always traverses the edge). The existing `revisionsExhausted` guard + `→blocked` edge remain as the now-*fed* backstop — belt and suspenders both live. The **delegate loop is untouched**; the separate `.exarchos.yml max-fix-cycles`-not-injected latent bug (`resolve.ts:356` resolved, never injected — delegate uses the hardcoded `hsm-definitions.ts:183` value) is **out of scope**, noted as a standalone follow-up.
...
- `serialize_merge` accepts `dryRun`, **defaults to dry-run** (INV-5c); the **handler short-circuits the lease claim/release on dry-run** (the exported `serializeMerge` handler branch, `merge-serializer.ts:321`, before the `merge_requested` append at `:250`) so no `worktree.merge_requested`/`merge_executed` lease events are appended and it returns the planned effect. (`autoEmits` metadata at `registry.ts:2956-2959` is descriptive only — a closed `'always'|'conditional'` enum — and is not the control point.)
...
**Acceptance criteria:**
- `prepare_review scope:plan` emits a counted `plan-review-dispatched` event **only on a re-dispatch** (the initial review emits none → revision 0); `planReview.revisionCount` folds `+1` per event through the existing projection, so `revisionCount` = re-dispatches = revisions. The `isRevision`-edge emission (`state-machine.ts:872-881`) is retired as a counter source **for the standard `plan-review → plan` edge only** — so exactly one producer on the standard path, no double-count when the prescribed flow also transitions.
...
- **Overhaul loop left intact (not converged onto the seam):** the `overhaul-plan-review` phase is a **human checkpoint** (`playbooks.ts:1005-1023`) that never dispatches through `prepare_review`, so its `isRevision`-edge counter (`hsm-definitions.ts:552-561`) is **preserved** — the emission retirement is scoped to the standard `plan-review → plan` edge only. A regression asserts the overhaul loop's edge-based bound still fires, so the Sentry-regression class cannot recur on **either** track.
...
**DR-1.** Add `surface?: 'worktree'` to `ToolAction` (`registry.ts:308-377`), set on the seven declarations. Typed `outputSchema`s replace `EnvelopeSchema(z.unknown())` at `:2871/2886/2921/2969/3327/3348/3377`, shape-derived from the real handler returns in `verbs/worktree/handlers.ts`/`merge-serializer.ts`. The two hardcoded tables in `dispatch.parity.test.ts` become one iteration over `TOOL_REGISTRY` filtered by `surface === 'worktree'`, asserting typed schema + annotations + real-handler-output validation + CLI≡MCP parity. `serialize_merge` schema (`:2942`) gains `dryRun`; the **handler** (`handlers.ts:347` → `serializeMerge` at `merge-serializer.ts:321`) short-circuits the lease claim/release before the `:250` append when `dryRun` (autoEmits metadata unchanged). `mutable`-as-hard-gate lands in `handleAcquireWorktree` (`handlers.ts:163`). `adapters/mcp.ts` added to Integration Points.

**DR-2.** `buildPlanReviewProvisioning` (`prepare-review.ts:176-210`) — currently pure — uses the `eventStore` already passed to `handlePrepareReview` (unused on the plan branch today) to: (1) read prior `plan-review-dispatched` events for the feature; (2) if this is the **initial** dispatch (none prior), provision and emit no counter event (revision 0); (3) else if the folded `revisionCount >= maxPlanRevisions` (resolved by the seam itself via `resolveConfig(args.repoRoot)` — `resolve.ts:357` — defaulting to `DEFAULT_MAX_PLAN_REVISIONS` when `repoRoot` is absent, matching the guard's value; **not** the transition-handler's `tools.ts:704` state injection, which the seam cannot see), return a park-at-`blocked` envelope and provision nothing; else (4) append one `plan-review-dispatched` event (INV-1: `revisionCount` folds `+1` per event, `workflow-state-projection.ts`) and provision. The emission carries a **deterministic idempotency key** (`${featureId}:plan-review-dispatch:${ordinal}`) that dedups a **same-ordinal** re-append at the storage layer (INV-8) — e.g. a store-internal append retry within one invocation. It does **not** make a full handler re-invocation idempotent: a retry after a committed append recomputes a higher `ordinal` from the durable count → new key → a fresh re-dispatch is counted (single-miscount exposure only on a crash between commit and response). No `operationId` schema field is added (that would require a `registry.ts` edit this DR disclaims); genuine re-invocation idempotency via a client token is a noted follow-up. Retire the `isRevision`-edge `plan-revision` emission (`state-machine.ts:872-881`) as a counter source **only for the standard `plan-review → plan` edge**; the shared block's `overhaul-plan-review → overhaul-plan` case is preserved. No delegate-side file is touched.
...
- `servers/exarchos-mcp/src/verbs/worktree/dispatch.parity.test.ts`, `handlers.ts`, `merge-serializer.ts` — registry-driven rewrite + lease-aware dry-run + `mutable` gate (DR-1).
- `servers/exarchos-mcp/src/verbs/team/prepare-review.ts` (stateful `buildPlanReviewProvisioning`), `views/workflow-state-projection.ts` (fold the seam event), `workflow/state-machine.ts` (retire `isRevision` emission as counter source), `workflow/guards.ts` (backstop unchanged) — DR-2.
...
**Files:** `registry.ts` (`ToolAction.surface`; sites `:2871/2886/2921/2969/3327/3348/3377`; descriptions), `verbs/worktree/schemas.ts`, `handlers.ts`, `merge-serializer.ts`, `verbs/worktree/worktree-schemas.test.ts`
...
**Files:** `registry.ts` (`:2942` schema), `verbs/worktree/handlers.ts` (`:347`,`:163`), `merge-serializer.ts` (`:184-238`), `orchestrate/worktree/*.test.ts`
...
**Files:** `verbs/worktree/dispatch.parity.test.ts`
...
**Boundary Touching:** true
**Implements:** DR-2
**Verification (high):** medium set + integration across the provisioning→projection seam. `buildPlanReviewProvisioning` reads prior `plan-review-dispatched` events, emits none on the initial dispatch, refuses past `maxPlanRevisions` (resolved via `resolveConfig(args.repoRoot)`, default when absent) with a park-at-`blocked` envelope, else emits one carrying a **deterministic idempotency key** (`${featureId}:plan-review-dispatch:${ordinal}`, INV-8) and provisions; `revisionCount` folds `+1` per event; the `isRevision`-edge emission (`state-machine.ts:872-881`) is retired as counter source **for the standard edge only** (the overhaul case preserved). **No delegate-side file, and no `registry.ts`, touched.**
**Files:** `verbs/team/prepare-review.ts` (`:176-210`), `views/workflow-state-projection.ts`, `workflow/state-machine.ts` (`:872-881`, edge-scoped), `verbs/team/prepare-review.test.ts`, `views/workflow-state-projection.test.ts`
**Expected tests:** `PrepareReviewPlan_InitialDispatch_EmitsNoCounter`, `PrepareReviewPlan_ReDispatch_EmitsCountedEvent`, `PrepareReviewPlan_PastCap_RefusesWithBlockedAffordance`, `RevisionCount_FoldsFromDispatchEvent_NotStandardEdge`, `PrepareReviewPlan_CrashRetrySameOrdinal_IdempotentByKey`
...
**Boundary Touching:** true
**Implements:** DR-2
**Verification (high):** medium set + integration. Prove the bypass is closed (re-provision without transition still counts + refuses at cap); the off-by-one (`N` permits exactly N cycles; initial is revision 0; survives consumes zero); the delegate loop is unchanged (its breaker/escalation tests pass as-is); and the **overhaul loop's edge-based bound still fires** (it is left intact, not converged onto the seam).
**Files:** `verbs/team/prepare-review.ts`, `verbs/team/prepare-review.test.ts`, `workflow/state-machine.test.ts` (delegate-unchanged + overhaul-edge-still-fires assertions)
**Expected tests:** `PlanReview_ReprovisionWithoutTransition_StillCountedAndCapped`, `PlanReview_OffByOne_PermitsExactlyNCycles`, `PlanReview_SurvivesVerdict_ConsumesZero`, `Delegate_FixCycleLoop_Unchanged`, `Overhaul_EdgeBound_StillFires`
...
**Files:** `views/tools.ts` (`:522`), `pipeline-view.ts` (`:28`), `views/composite.ts` (`:99`), `verbs/worktree/handlers.ts` (`:321`), `registry.ts` (`:3324` worktrees schema + pipeline default cap), `views/tools.pipeline.test.ts`, `views/handlers.test.ts`, `views/composite.output-token-hint.test.ts`
