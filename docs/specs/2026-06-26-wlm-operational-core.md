...
**Module layout** (extend `servers/exarchos-mcp/src/verbs/worktree/`, created by the foundation):
...
- `verbs/worktree/projections/worktrees.ts` — extend `worktrees@v1` with the integration-ref-keyed `inFlightMerges` map folding the merge pair, plus folding probe-emitted `orphan_detected`/`released`.
- `verbs/worktree/pure/probe.ts` — process+fs scan behind an **injected process source**; symlink-canonicalized containment; protected-ancestry subtraction (DR-5).
- `verbs/worktree/git-retry.ts` — injected backoff/jitter/sleep seam + `index.lock` retry wrapper + burst jitter (DR-8); the sleep seam is reused by Task 004's bounded `wait`.
- `verbs/worktree/merge-serializer.ts` — the optimistic-lease loop (fold → wait-for-slot → OCC claim → fresh-HEAD re-read → unchanged `merge_orchestrate` → plain release).
- **Dispatch wiring (DOA-class guard):** `verbs/worktree/handlers.ts` gains `handleSerializeMerge`, `handleViewPs`, `handleViewWait`; `verbs/composite.ts` `ACTION_HANDLERS` gains `serialize_merge`; `views/composite.ts` `handleView` switch gains `case 'ps'`/`case 'wait'`. The orchestrate side is guarded by `OrchestrateActions_MatchCompositeHandlers_InSync` (registry.test.ts:801); the view side currently has only a *superset* guard (`views/describe.coverage.test.ts`), so this slice adds the missing `ViewActions_MatchCompositeHandlers_InSync` equality twin (Task 007). All new-verb tests run **through `handleView`/`handleOrchestrate`**, not the module in isolation.
...
- `verbs/composite.ts` + `views/composite.ts` — dispatch wiring (DR-4/7).
...
Maps every task to DR-N from `## Design & Rationale`. All work extends `servers/exarchos-mcp/src/verbs/worktree/` (PR #1628).
...
**Files:** `servers/exarchos-mcp/src/verbs/worktree/pure/probe.ts`, `servers/exarchos-mcp/src/verbs/worktree/pure/probe.test.ts`
...
**Files:** `servers/exarchos-mcp/src/views/composite.ts`, `servers/exarchos-mcp/src/verbs/worktree/handlers.ts`, `servers/exarchos-mcp/src/verbs/worktree/handlers.test.ts`, `servers/exarchos-mcp/src/verbs/worktree/manager.ts`
...
**Verification (medium):** scoped tests + `check_test_adequacy` kill-probe (test-after).
**Files:** `servers/exarchos-mcp/src/verbs/worktree/git-retry.ts`, `servers/exarchos-mcp/src/verbs/worktree/git-retry.test.ts`
...
**Files:** `servers/exarchos-mcp/src/verbs/worktree/merge-serializer.ts`, `servers/exarchos-mcp/src/verbs/worktree/merge-serializer.test.ts`, `servers/exarchos-mcp/src/verbs/composite.ts`, `servers/exarchos-mcp/src/verbs/worktree/handlers.ts`
...
**Files:** `servers/exarchos-mcp/src/verbs/worktree/recovery.test.ts`, `servers/exarchos-mcp/src/verbs/worktree/merge-serializer.ts`
...
- **Wave 3 (parallel):** Task 004 (view dispatch; needs 001+003+005), Task 006 (lease serializer; needs 001+003+005). **Coordination:** both edit `verbs/worktree/handlers.ts` (004 adds `handleViewPs`/`handleViewWait`; 006 adds `handleSerializeMerge`) — additive, non-overlapping functions; if run in separate worktrees, rebase/merge `handlers.ts` carefully (or sequence 006 → 004).
