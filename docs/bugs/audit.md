```bash
  ...
  - Per-task gate checks — RESOLVED. handleTaskComplete enforces gate.executed check before completion — queries event store for `gate.executed` with matching `gateName: 'tdd-compliance'`, `taskId`, and `passed: true`. No bypass path exists. Delegation skill invokes `check_tdd_compliance` orchestrate action which auto-emits the gate event; handleTaskComplete independently verifies event presence.
