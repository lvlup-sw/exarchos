...
**Status:** Historical snapshot. PR-A merged into #1369; the counts below reflect
the **pre-PR-A** state of the tree (the legacy wrappers + `formatResult` itself
have since been deleted). Counts have not been re-reconciled post-merge —
CodeRabbit minor on PR #1369 flagged some A1.* totals as inconsistent across
the document; they remain as the original audit emitted them. Trust the
"port"/"discard" decision column over the bullet-list totals; the actual
post-merge tree state is authoritative via `git log` on commit `9af89d9f`.
