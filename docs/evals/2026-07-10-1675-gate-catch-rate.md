...
**Headline:** on this corpus every gate is a clean separator — **catch rate 100% (5/5) and false-positive rate 0% (0/5) for all five gates, with zero invalid cells** — so **no gate is flagged for redesign or removal**. The result is a *floor* verdict at N=5, not a proof of perfection: §Verdict states the confidence bound and the one known precision caveat, and §Honest scope states what the small hermetic fixtures structurally cannot see.
...
\* `check_mock_boundary` keeps its verdict but carries a documented identifier-heuristic precision caveat (§Verdict).
