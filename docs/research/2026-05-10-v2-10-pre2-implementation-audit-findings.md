...
| #1342 P1.E | Marten-followups Wave C | Add CI grep gate `scripts/check-begin-immediate-substrate.sh` enforcing the SQLite `BEGIN IMMEDIATE` primitive lives only inside `storage/` and `event-store/`. Wired into `.github/workflows/ci.yml` `grep-gates` job. | **Done** (Wave C). |
