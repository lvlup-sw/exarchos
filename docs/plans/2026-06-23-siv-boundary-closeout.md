...
Add a second boundary leg under the lint gate in `static-analysis.ts` (sibling to the shipped Layer-A `runBoundaryLint` dependency-cruiser leg). A **Semgrep-driven taint query** — `runRawIoTaint` — that flags results of `JSON.parse` / `response.json()` / `req.body` / `fs.read*` **not immediately consumed by a registered parse fn** (`parsers:` convention, resolved like a toolchain entry — see Task 1a decision below), AND forbids downstream `as Brand` / `as any` casts (Zod `.brand()` is compile-time-only; one stray cast defeats it). Semgrep is the first-class engine across runtimes (guarantee agnostic, impl per-runtime — INV-4); absent the engine or its committed ruleset it SKIP/advisory-degrades like Layer A, and only a real finding (exit 1) hard-FAILs. Emit `gate.executed`. Document the degrade path in the module JSDoc.
