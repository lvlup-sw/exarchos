...
vi.mock('../../../src/projections/session/manifest.js', async (importOriginal) => {
  const actual = await importOriginal<typeof import('../../../src/projections/session/manifest.js')>();
  return {
    ...actual,
...
describe('session-end command', () => {
  ...
  describe('extraction', () => {
    ...
    it('handleSessionEnd_ValidTranscript_UpdatesManifestWithCompletion', async () => {
      const { handleSessionEnd } = await import('../../../src/lifecycle/session-end.js');
      const { parseTranscript } = await import('../../../src/projections/session/transcript-parser.js');
      const { writeManifestCompletion } = await import('../../../src/projections/session/manifest.js');
