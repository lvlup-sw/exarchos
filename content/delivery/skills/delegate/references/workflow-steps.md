...
{{SPAWN_AGENT_CALL agent="reviewer" description="Plan review voter 1" prompt="<provisioned review prompt>"}}
```
...
- Settle the batch as usual (`settle`) — the hook's own gate runs are not settlement, and a task is complete only once settlement says so
<!-- /requires -->
