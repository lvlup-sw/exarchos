...
import { daysBetween } from '../src/architecture/waiver-ledger.js';
import {
  VACUITY_ALLOWLIST,
...
describe('DR-4: the vacuity allowlist expiry is enforced, not advisory', () => {
  ...
  it('OutputSchemaExpiry_UnexpiredEntry_PassesTheGuard', () => {
    ...
    // any deadline — green.
    //
    // TASK 093 re-dated this arm. It used to be taken at the anchor, on the
    // premise that every waiver lived until then; the schedule is staggered now,
    // so the last day the WHOLE seed is live is the FIRST cohort's slot, not the
    // last. Derived from the schedule rather than written, because the day this
    // assertion needs is a consequence of the seed and would otherwise have to
