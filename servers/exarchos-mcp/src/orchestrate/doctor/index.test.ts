...
describe('handleDoctor — verification-toolchain roster (task 009)', () => {
  it('HandleDoctorWithChecks_RosterIncludesVerificationToolchain_FifteenChecks', async () => {
    ...
    // Assert — the roster ships exactly 15 checks now (Task 017 added
    // onramp-block-drift + retired-hooks-present), and verification-toolchain is
    // present (by category + name) in the dispatched output, not just the export.
    expect(ALL_CHECKS).toHaveLength(15);
    expect(result.success).toBe(true);
    const data = result.data as { checks: CheckResult[] };
    expect(data.checks).toHaveLength(15);
