...
// `git reset --keep <recoveryPointSha>` recovery a no-op in production: a remote
  ...
  return async ({ sourceBranch, targetBranch, strategy }) => {
    ...
    switch (strategy) {
      ...
      case 'rebase': {
        ...
        // The executor's recovery path is `git reset --keep <recoveryPointSha>` on
        ...
        try {
          ...
          // so the executor's reset --keep <recoveryPointSha> targets the right ref.
