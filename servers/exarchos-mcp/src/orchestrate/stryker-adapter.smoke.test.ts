...
import { parseMutationReport } from './mutation-adequacy.js';
...
import {
  ...
  EMPTY_REPORT,
  MAX_MUTATE_FILES,
} from '../../scripts/stryker-adapter.mjs';
...
describe('stryker-adapter pure helpers', () => {
  ...
  it('isMutatableServerSource restricts to changed servers/exarchos-mcp/src/** production files', () => {
    expect(isMutatableServerSource('servers/exarchos-mcp/src/foo.ts')).toBe(true);
    expect(isMutatableServerSource('servers/exarchos-mcp/src/foo.test.ts')).toBe(false);
    expect(isMutatableServerSource('servers/exarchos-mcp/src/foo.d.ts')).toBe(false);
    expect(isMutatableServerSource('servers/exarchos-mcp/src/foo.type-test.ts')).toBe(false);
    expect(isMutatableServerSource('servers/exarchos-mcp/src/foo.bench.ts')).toBe(false);
    expect(isMutatableServerSource('servers/exarchos-mcp/scripts/other.ts')).toBe(false);
  ...
  it('computeMutateGlobs filters to still-existing, mutatable files and strips the server prefix', () => {
    const changed = [
      'servers/exarchos-mcp/src/foo.ts', // qualifies
      'servers/exarchos-mcp/src/foo.test.ts', // excluded: test file
      'servers/exarchos-mcp/src/deleted.ts', // excluded: no longer exists
      'docs/guides/toolchain-resolution.md', // excluded: outside server src
    ];
    const exists = (f: string) => f !== 'servers/exarchos-mcp/src/deleted.ts';
  ...
  it('computeMutateGlobs caps at MAX_MUTATE_FILES (mutant-count bound) and reports truncation', () => {
    const changed = Array.from(
      ...
      (_, i) => `servers/exarchos-mcp/src/f${String(i).padStart(3, '0')}.ts`,
    );
...
describe('resolveVerificationRuntime mutation-field provenance (DR-7)', () => {
  it('resolves this repo\'s .exarchos.yml adapter entry, not the built-in npx fallback', () => {
    ...
    expect(runtime.mutation).toBe('node servers/exarchos-mcp/scripts/stryker-adapter.mjs');
  ...
  it('config-tier mutation: beats the built-in node registry default generically', () => {
    ...
    try {
      ...
      const runtime = resolveVerificationRuntime(tmp, {
        loadConfig: () => ({
          config: { mutation: 'node servers/exarchos-mcp/scripts/stryker-adapter.mjs' },
