...
// ─── Evidence, per (hop, ROW) ────────────────────────────────────────────────
//
// ## Why this is not keyed by hop (task 066, and the defect it removes)
//
// Task 025 shipped this field as `Record<CensusHop, HopEvidence>` — the shape
// P05-05's `HOP_AUTHORITIES` established. Task 026 then measured two of the eight
// rows LIVE (`scripts/authority-live-proof.ts` rebuilds the `cli-surface` and
// `event-catalog` rows from source and runs the unmodified census over them) and
// found it could not record what it had proved: flipping the hop-level
// `authority`/`binding` values away from `declared-row` would have claimed live
// evidence for ALL EIGHT rows when six still had none. 026 correctly refused, and
// reported the shape as the defect.
//
// A hop-level value is not merely imprecise; it is an INHERITANCE CHANNEL. Any
// row's evidence could be read off it, so the over-claim 026 declined to make was
// permanently one edit away for the next author. The fix is therefore not a
// second field or a runtime check — it is to make the inherited entry
// UNREPRESENTABLE:
//
//   • {@link RowHopEvidence} is parameterised by the very key it is filed under.
//     An entry for `cli-surface`/`authority` has TYPE `boundary: 'cli-surface'`,
//     so moving it into another row's slot fails to compile. Copy-paste, spread
//     and a shared hop-level constant are all rejected by the same tooth — the
//     last of those cannot even be WRITTEN, because no entry may leave its
//     `boundary` unstated. See `_InheritedEvidence_FailsCompile` at the foot of
//     this file, with its positive control.
//   • {@link BOUNDARY_HOP_EVIDENCE} is a mapped type over
//     `ContractBoundaryId × CensusHop`, so it is TOTAL: a new boundary or a new
//     hop is a compile error until its evidence is stated, and there is no
//     default, fallback or partial arm for a lookup to widen from.
//   • {@link auditRowEvidence} re-proves the same facts at runtime for a table
//     that arrived as data (a fixture, a store, a JSON round trip) and therefore
//     never met the compiler.
//
// ## What counts as `live-measurement`, stated so the six stay honest
//
// `declared-row` is NOT independent evidence about the tree — it is the committed
// measurement task 024 made, and P05-05's ban on the value `self` is the same
// honesty. `live-measurement` is the strictly stronger claim, and it is earned
// ONLY by a shipped oracle that a reviewer can re-run now: a named module, a named
// exported entrypoint, and the tree paths it reads ({@link LiveOracle}). A row
// whose `measured` prose says "re-measured on <date>" has a human's transcription,
// not an oracle — `response-shape` and `sdk-generation` both read that way and
// both remain `declared-row`, because nothing in the tree rebuilds those rows
// today. Six rows are visibly weaker than two, for a stated reason.
...
// ─── The evidence audit (the runtime twin of the type-level tooth) ───────────
//
// The compiler already forbids an inherited entry in SOURCE. This function
// re-proves the same facts for a table that arrived as DATA and therefore never
// met the compiler — a fixture, a store, a JSON round trip. It mirrors
// `checkTopologyTotality`, which takes `readonly unknown[]` for exactly that
// reason, and it introduces no error code: every finding's `kind` is
// {@link CensusFindingKind}, imported from P05-05.
...
/** Every hop of every row is stated — totality on the second axis too. */
