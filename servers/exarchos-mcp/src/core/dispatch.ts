...
function extractSingleMissingRequiredField(
  ...
  //     type) so we don't divert wrong-type errors into elicitation, AND
  //   - `received === 'undefined'` — belt-and-braces guard against the
  //     wrong-type case (`featureId: 42` → received: 'number'). Elicitation
  //     only fires on actually-absent fields; wrong-type values surface as
