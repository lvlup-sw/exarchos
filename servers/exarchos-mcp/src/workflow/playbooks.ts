...
register({
  ...
  humanCheckpoint: false,
  compactGuidance:
    'You are authoring the Decomposition section of the unified docs/specs/ artifact — tasks tracing to the DR-N source in the SAME doc\'s Design & Rationale section (traceability is internal; no second file). Use exarchos_workflow update to record artifacts.plan (the docs/specs/ path — the planArtifactExists guard key), then transition to plan-review. Three-stage decomposition: (1) identify logical units, (2) define concrete tasks with inputs/outputs, (3) build the parallelization plan. Each task is 2-5 min, stamped with a riskTier and Implements: DR-N. Anti-pattern: monolith tasks; a separate docs/plans/ file. Escalate: ambiguous requirements that block decomposition. Follow the phase-compression runbook to create self-contained context packages per task.',
});
