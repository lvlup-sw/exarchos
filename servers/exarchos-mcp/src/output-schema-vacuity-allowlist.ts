...
//      `scripts/output-schema-ratchet-guard.ts` reads the owners off the frozen
//      seed (this map ∪ `VACUITY_RETIRED`) and orders them by seeded cohort
//      size, smallest due first. Deriving it from the SEED and not from today's
//      allowlist is deliberate: the seed key set is the quantity the digest pins,
//      so paying an entry down can never shuffle a rank or move another team's
//      deadline.
//
