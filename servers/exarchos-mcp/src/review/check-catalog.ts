    ...
    // ── Structural Complexity (SC) ──────────────────────────────────────────
    {
      ...
      checks: [
        ...
        },
        {
          ...
          execution: 'heuristic',
          severity: 'MEDIUM',
          description: 'Function/method declarations with too many parameters suggest a function does too much or needs an options object',
          fileGlob: '*.ts',
          threshold: 5,
          remediation: 'Group related parameters into an options/config object.',
          falsePositives: 'Destructured parameters that are visually long but logically a single options object. Call sites with many arguments are not flagged — only declarations.',
        },
      ],
    },
    ...
    // ── Resilience (RS) ─────────────────────────────────────────────────────
    {
      ...
      checks: [
        ...
        },
        {
          ...
          execution: 'heuristic',
          severity: 'MEDIUM',
          description: 'Retry loops without a bounded counter can cause infinite retries on persistent failures',
          fileGlob: '*.ts',
          remediation: 'Add a maximum retry count, exponential backoff, and a circuit breaker or timeout.',
          falsePositives: 'Retry loops that include a bounded counter (e.g., `while (retries < maxRetries)`) are safe.',
        },
      ],
    },
  ],
