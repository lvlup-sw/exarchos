...
function timeoutsByName(): Map<string, { testTimeout?: number; hookTimeout?: number }> {
  return new Map(
    projects()
      .filter((p) => typeof p.test?.name === 'string')
      .map((p) => [
        p.test?.name as string,
        { testTimeout: p.test?.testTimeout, hookTimeout: p.test?.hookTimeout },
      ]),
  );
}
