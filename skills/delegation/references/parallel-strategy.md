...
Teammates use Claude Code's native shared task list for claim/complete tracking. Exarchos persistent state is updated via the `TeammateIdle` quality gate hook.
...
| Waiting | `TaskOutput({ block: true })` | `TeammateIdle` hook fires |
