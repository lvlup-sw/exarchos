...
How the exarchos quality review phase delegates general backend quality checks to the axiom plugin while retaining domain-specific checks.
...
/exarchos:review (quality-review stage)
    │
    ├── Invoke axiom:audit (general backend quality)
    │   └── Returns: findings[] + verdict (CLEAN | NEEDS_ATTENTION)
    │
    ├── Run exarchos-specific checks (not in axiom)
    ...
    │
    ├── Merge findings (axiom + exarchos-specific)
    │
    ├── Compute verdict
...
|---------|-------|-----------|
| DIM-1: Topology | Axiom plugin | General backend quality |
| DIM-2: Observability | Axiom plugin | General backend quality |
| DIM-3: Contracts | Axiom plugin | General backend quality |
| DIM-4: Test Fidelity | Axiom plugin | General backend quality |
| DIM-5: Hygiene | Axiom plugin | General backend quality |
| DIM-6: Architecture | Axiom plugin | General backend quality |
| DIM-7: Resilience | Axiom plugin | General backend quality |
...
## Invoking Axiom from Quality Review

During the quality review stage, after spec-review passes:

1. **Run axiom:audit** for the feature scope (diff files):
   ```text
   axiom:audit --scope [changed files from diff]
   ```

2. **Translate axiom findings** to exarchos finding format:
   - Axiom `dimension` → map to exarchos dimension (DIM-1 → general-topology, etc.)
   - Axiom `severity` → same tiers (HIGH, MEDIUM, LOW)
   - Axiom `evidence` → same format (file:line)
...
| Axiom Verdict | Exarchos-Specific Findings | Combined Verdict |
|--------------|---------------------------|-----------------|
| CLEAN | None | APPROVED |
...
- Axiom plugin created in exarchos repo under `axiom/` with 45 structural validation tests
- 6 composable skills: scan, critique, harden, distill, verify, audit
- Shipped in #1023

### Phase 2 (Current): Delegation
- Axiom extracted to standalone repo: [lvlup-sw/axiom](https://github.com/lvlup-sw/axiom)
- Install independently: `claude plugin add lvlup-sw/axiom`
- Quality review delegates general checks to axiom:audit
...
- Quality review is purely: axiom:audit + exarchos-specific checks + verdict
- Convergence dimensions map cleanly to axiom dimensions + exarchos dimensions
