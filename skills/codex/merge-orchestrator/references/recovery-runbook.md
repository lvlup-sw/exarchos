...
6. **Do not** re-dispatch `merge_orchestrate` — the merge is now done manually. Mark the workflow's `mergeOrchestrator.phase` as `completed` via `mcp__exarchos__exarchos_workflow set` and emit a `merge.executed` event manually:
