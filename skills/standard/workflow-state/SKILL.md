...
Use `exarchos:exarchos_workflow` with `action: "get"` and `featureId`:
...
For context restoration after summarization, use `exarchos:exarchos_workflow` with `action: "get"` and `featureId`. This outputs a minimal summary suitable for rebuilding orchestrator context.
...
3. Update state via `exarchos:exarchos_workflow` with `action: "update"` to match git truth
...
3. **Check state**: Use `exarchos:exarchos_workflow` with `action: "get"` with `featureId: "user-authentication"`

4. **Resume after context loss**: Use `exarchos:exarchos_workflow` with `action: "get"` with `featureId: "user-authentication"` to get context restoration output
