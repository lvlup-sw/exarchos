...
Use `mcp__plugin_exarchos_exarchos__exarchos_orchestrate({ action: "describe", actions: ["merge_orchestrate"] })` for the argument schema. Use `mcp__plugin_exarchos_exarchos__exarchos_workflow({ action: "describe", playbook: "feature" })` for the full feature-workflow phase playbook (which includes `merge-pending`). Use `mcp__plugin_exarchos_exarchos__exarchos_event({ action: "describe", eventTypes: ["merge.preflight", "merge.executed", "merge.rollback"] })` for event payload shapes.
