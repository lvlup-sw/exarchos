...
Use `mcp__plugin_exarchos_exarchos__exarchos_workflow` with `action: "update"` with `featureId` and either `updates`, `phase`, or both:
...
- `phase` → `mcp__plugin_exarchos_exarchos__exarchos_workflow` with `action: "transition"` and `toPhase: "<target>"`. Transitions are HSM-validated and emit transition events.
...
```json
{
  ...
  "error": {
    ...
    "data": {
      ...
      "alternateWritePath": "Use `mcp__plugin_exarchos_exarchos__exarchos_workflow.transition({featureId, toPhase})` — phase changes are HSM-validated and emit transition events."
    }
  }
}
```
...
2. **After design phase**: Use `mcp__plugin_exarchos_exarchos__exarchos_workflow` with `action: "update"` with:
