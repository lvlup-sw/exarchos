...
| Sink | Field |
|------|-------|
| `.Codex-plugin/plugin.json` | `.version`, `.metadata.compat.minBinaryVersion` |
...
```bash
git add package.json package-lock.json manifest.json .Codex-plugin/plugin.json \
...
```bash
cd ~/.Codex/plugins/marketplaces/lvlup-sw
...
```bash
gh api repos/lvlup-sw/.github/contents/.Codex-plugin/marketplace.json --jq '.content' \
...
## Why not `Codex plugin update`?

The plugin installer resolves versions through the npm CDN, which caches aggressively and frequently serves stale tarballs even after `npm publish` succeeds. Step 7 above (`sync-marketplace.sh`) bypasses the CDN entirely by pointing your local marketplace clone at the new version directly. Use that instead of `Codex plugin update` whenever you need the new bits in your current session.
