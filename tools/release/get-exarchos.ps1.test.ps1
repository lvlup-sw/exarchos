...
vitest wrapper at tools/release/get-exarchos.ps1.test.ts spawns `pwsh` to cover
the end-to-end dry-run path.

Run locally:
  Invoke-Pester -Path tools/release/get-exarchos.ps1.test.ps1 -Output Detailed
#>
...
Describe 'get-exarchos.ps1' {
    ...
    Context 'GetExarchos_PlatformDetection_Windows_arm64' {
        ...
        # so `tools/release/build-binary.ts` does not produce `exarchos-windows-arm64.exe`.
