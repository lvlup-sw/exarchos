...
**Detail:** Two authorities decided what an event name may be, and **as measured on 2026-08-06 they disagreed on 25 of 171** live names. Found by task 014, measured on the runtime path by task 015, and recorded by 015 as an owned, dated, two-way-checked concession rather than fixed — because fixing it changes a public runtime seam.

> **CLOSED by [#1764](https://github.com/lvlup-sw/exarchos/issues/1764) task 075.** The two forms were collapsed onto the DR-3 grammar, so the live divergence now derives <!-- measured: event-name-pattern-divergence -->0<!-- /measured -->. The 25 above is retained as a DATED measurement rather than re-derived to zero, because zeroing it in place would make the sentence describe a defect that no longer exists while reading as though it never did. Migration: [`docs/migrations/2026-08-10-event-name-grammar.md`](../migrations/2026-08-10-event-name-grammar.md).
