...
const MAP = {
  ...
  workflow:     M('workflow', 'State machines, phase contracts, guards, checkpoint — L4 itself.'),
  topology:     M('workflow', 'Phase-contract loader and staleness signal: workflow-primitive data.'),
  ...
  contract:     M('contract', 'Authority collection, digests, IR and admission — the contract half of the L5 split.'),
  schemas:      M('contract', 'Envelope and totality schemas are contract surface.'),
  sdk:          M('contract', 'Brand and seam types are the contract boundary the SDK exposes.'),
  shared:       M('contract', 'Shared validation is contract-shaped and consumed across the split.'),
  dispatch:     M('dispatch', 'Caller identity, dispatch context, elicitation dispatch — the dispatch half of the L5 split.'),
  core:         M('dispatch', 'context.ts and dispatch.ts ARE the single dispatch function L5 describes.'),
