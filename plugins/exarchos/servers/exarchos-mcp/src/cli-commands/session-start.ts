 ...
 */
async function collectContextDocuments(checkpoints: ReadonlyArray<CheckpointData>): Promise<string | undefined> {
  const contextDocuments: string[] = [];

  for (const cp of checkpoints) {
    if (!cp.contextFile) continue;
    const content = await readAndDeleteContextFile(cp.contextFile);
    if (content) {
...
export async function handleSessionStart(
  ...
  if (checkpoints.length > 0) {
    ...
    const contextDocument = await collectContextDocuments(checkpoints);
