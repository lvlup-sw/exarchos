...
describe('Package scaffold', () => {
  describe('package.json', () => {
    it('has required fields: name, version, type, main', () => {
      ...
      expect(pkg.name).toBe('@lvlup-sw/workflow-state-mcp');
      expect(pkg.version).toBe('1.0.0');
      expect(pkg.type).toBe('module');
      expect(pkg.main).toBe('dist/index.js');
    });

    it('has required scripts: build, test, test:run, typecheck', () => {
      const raw = readFileSync(packageJsonPath, 'utf-8');
      const pkg = JSON.parse(raw);
  ...
  describe('plugin.json', () => {
    ...
    it('has required fields: name, description, version', () => {
      ...
      expect(plugin.name).toBe('workflow-state');
  ...
  describe('mcp-servers.json', () => {
    ...
    it('has workflow-state server configuration', () => {
      const raw = readFileSync(mcpServersJsonPath, 'utf-8');
      const config = JSON.parse(raw);

      expect(config['workflow-state']).toBeDefined();
      expect(config['workflow-state'].type).toBe('stdio');
      expect(config['workflow-state'].command).toBe('node');
    });
    ...
    it('has correct server entry point path', () => {
      ...
      const args = config['workflow-state'].args;
      expect(args).toBeInstanceOf(Array);
      expect(args).toHaveLength(1);
      expect(args[0]).toContain('servers/workflow-state-mcp/dist/index.js');
    });
    ...
    it('has WORKFLOW_STATE_DIR environment variable', () => {
      ...
      expect(config['workflow-state'].env).toBeDefined();
      expect(config['workflow-state'].env.WORKFLOW_STATE_DIR).toBeDefined();
    });
  });
