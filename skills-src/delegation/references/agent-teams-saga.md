...
| Constraint | Impact |
|------------|--------|
| **No session resumption** for teammates | Teammates are ephemeral. On restart, `/exarchos:rehydrate` surfaces orphaned teams in the rehydration document but cannot restore them. Spawn new teammates if delegation is incomplete. |
