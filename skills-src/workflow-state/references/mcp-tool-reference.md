...
- **`{{COMMAND_PREFIX}}rehydrate <featureId>`** — Re-injects workflow state, phase playbook, and behavioral guidance into the current context. Run this at the start of any session that resumes a workflow. Replaces the former auto-running SessionStart hook.
