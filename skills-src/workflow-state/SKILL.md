...
Use `{{MCP_PREFIX}}exarchos_workflow` with `action: "update"` with `featureId` and either `updates`, `phase`, or both:
...
- `phase` → `{{MCP_PREFIX}}exarchos_workflow` with `action: "transition"` and `toPhase: "<target>"`. Transitions are HSM-validated and emit transition events.
...
```json
{
  ...
  "error": {
    ...
    "data": {
      ...
      "alternateWritePath": "Use `{{MCP_PREFIX}}exarchos_workflow.transition({featureId, toPhase})` — phase changes are HSM-validated and emit transition events."
    }
  }
}
```
...
3. Update state via `{{MCP_PREFIX}}exarchos_workflow` with `action: "update"` to match git truth
...
2. **After design phase**: Use `{{MCP_PREFIX}}exarchos_workflow` with `action: "update"` with:
