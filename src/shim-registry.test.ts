...
const TEMP_ROOTS = ['src', 'servers/exarchos-mcp/src'];
...
function rendererEntry(over: Partial<ShimEntry> = {}): ShimEntry {
  return {
    ...
    file: 'servers/exarchos-mcp/src/agents/adapters/newharness.ts',
    runtime: 'newharness',
...
const RENDERER_PATH = 'servers/exarchos-mcp/src/agents/adapters/newharness.ts';
...
describe('discoverRenderers — marker independence (the DR-14 defect)', () => {
  ...
  it('DiscoverRenderers_RendererOutsideAdaptersDirectory_IsStillDiscovered', () => {
    const elsewhere = 'servers/exarchos-mcp/src/launcher/newharness-renderer.ts';
...
describe('discoverRenderers — false-positive guard', () => {
  ...
  it('DiscoverRenderers_PortDeclarationModule_YieldsNoDiscovery', () => {
    const portModule =
      ...
      `}\n`;
    withTempRepo(
      { 'servers/exarchos-mcp/src/agents/adapters/types.ts': portModule },
      (repoRoot) => {
  ...
  it('DiscoverRenderers_PortConsumerInGenericPosition_YieldsNoDiscovery', () => {
    ...
    withTempRepo(
      { 'servers/exarchos-mcp/src/agents/generate-agents.ts': consumer },
      (repoRoot) => {
        expect(discoverRenderers({ repoRoot, roots: TEMP_ROOTS })).toEqual([]);
      },
    );
  ...
  it('DiscoverRenderers_ImplementorWithoutRenderMember_YieldsNoDiscovery', () => {
    ...
    withTempRepo(
      { 'servers/exarchos-mcp/src/agents/adapters/stub.ts': stub },
      (repoRoot) => {
        expect(discoverRenderers({ repoRoot, roots: TEMP_ROOTS })).toEqual([]);
      },
    );
  ...
  it('DiscoverRenderers_TestFileShapedLikeRenderer_YieldsNoDiscovery', () => {
    withTempRepo(
      { 'servers/exarchos-mcp/src/agents/adapters/fake.test.ts': rendererSource() },
      (repoRoot) => {
        expect(discoverRenderers({ repoRoot, roots: TEMP_ROOTS })).toEqual([]);
      },
    );
  ...
  it('RendererSubject_IsThePortAndRenderMember_NotAPathConvention', () => {
    ...
    expect(RENDERER_SCAN_ROOTS).toContain('servers/exarchos-mcp/src');
  });
});
...
describe('DR-14 stale cover — a row whose artefact left the tree fails', () => {
  it('ShimRatchet_RegisteredRendererAbsentFromDisk_FailsMissingOnDisk', () => {
    ...
    withTempRepo(
      { 'servers/exarchos-mcp/src/agents/adapters/other.ts': rendererSource({ runtime: 'other' }) },
      (repoRoot) => {
        const renderers = discoverRenderers({ repoRoot, roots: TEMP_ROOTS });
        const result = verifyShimRatchet({
          registry: [rendererEntry()],
  ...
  it('ShimRatchet_StrayMarkerWithNoRow_StillFails', () => {
    ...
    withTempRepo(
      {
        'servers/exarchos-mcp/src/runtime/stray.ts':
...
describe('DR-14 live tree — the inventory reflects the shipped renderers', () => {
  ...
  const SHIPPED_RENDERERS: ReadonlyArray<readonly [string, string]> = [
    ['servers/exarchos-mcp/src/agents/adapters/claude.ts', 'claude'],
    ['servers/exarchos-mcp/src/agents/adapters/codex.ts', 'codex'],
    ['servers/exarchos-mcp/src/agents/adapters/copilot.ts', 'copilot'],
    ['servers/exarchos-mcp/src/agents/adapters/cursor.ts', 'cursor'],
    ['servers/exarchos-mcp/src/agents/adapters/opencode.ts', 'opencode'],
  ];
