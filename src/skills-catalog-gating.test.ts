 ...
 * sources that gate their design-time (`/ideate`, `/refactor`, `/debug`) or
 * evaluation-time (`/shepherd`) Constraints step on the retired flag were
 * re-pointed at catalog registration.
 *
 * This lint pins two invariants over the source-of-truth `skills-src/` tree
 * (not the rendered `skills/<runtime>/` variants, which vary by runtime
 * placeholder substitution and are regenerated by `build:skills` from these
 * same sources):
 *
