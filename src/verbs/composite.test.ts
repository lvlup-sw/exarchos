...
vi.mock('./team/prepare-delegation.js', () => ({
  handlePrepareDelegation: vi.fn(),
}));

vi.mock('./team/prepare-synthesis.js', () => ({
  handlePrepareSynthesis: vi.fn(),
}));

vi.mock('./vcs/assess-stack.js', () => ({
  handleAssessStack: vi.fn(),
}));

vi.mock('./gates/design-completeness.js', () => ({
  handleDesignCompleteness: vi.fn(),
}));

vi.mock('./gates/plan-coverage.js', () => ({
  handlePlanCoverage: vi.fn(),
}));


vi.mock('./gates/post-merge.js', () => ({
  handlePostMerge: vi.fn(),
}));

vi.mock('./team/prune-stale-workflows.js', () => ({
  handlePruneStaleWorkflows: vi.fn(),
}));

vi.mock('./team/request-synthesize.js', () => ({
  handleRequestSynthesize: vi.fn(),
}));

vi.mock('./tasks/finalize-oneshot.js', () => ({
...
vi.mock('./merge/merge-orchestrate.js', () => ({
  handleMergeOrchestrate: vi.fn(),
}));

vi.mock('../runtime/agents/handler.js', async (importOriginal) => {
