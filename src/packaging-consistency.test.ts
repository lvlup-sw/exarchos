...
describe('versioned packaging (Task 022, DR-4)', () => {
  it('versionCheck_AllSinksMatchRootPackageJson', () => {
    ...
    const mcpPkg = readJson('servers/exarchos-mcp/package.json');
    ...
    expect(mcpPkg.version, 'servers/exarchos-mcp/package.json .version').toBe(rootVersion);
    expect(serverVersionLiteral('servers/exarchos-mcp/src/index.ts'), 'index.ts SERVER_VERSION').toBe(
      rootVersion,
    );
    expect(
