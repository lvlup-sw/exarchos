---
name: synthesis
description: "Final workflow step: Create pull request from integration branch after reviews pass. Handles PR creation and merge confirmation."
---
...
---
Generated with Copilot CLI Orchestration Workflow
EOF
)"
```
...
```bash
~/.copilot/scripts/workflow-state.sh get docs/workflow-state/<feature>.state.json '.integration.branch'
~/.copilot/scripts/workflow-state.sh get docs/workflow-state/<feature>.state.json '.integration.mergedBranches'
```
...
```bash
~/.copilot/scripts/workflow-state.sh set docs/workflow-state/<feature>.state.json '.phase = "completed"'
```
...
1. Offer to address feedback by invoking delegation in PR-fixes mode
2. Delegation reads PR comments via:
   ```bash
...
- **'feedback'**: Continue to delegation in PR-fixes mode to address comments, then return here
