---
name: quality-review
description: "Stage 2 review: Assess code quality, SOLID principles, security, and maintainability. Runs after spec-review passes."
---
...
This skill runs in a review agent spawned by the orchestrator, not inline.

The orchestrator provides:
- State file path (preferred) OR design/plan paths
- Diff output from review-diff script (context-efficient)
...
~/.copilot/scripts/review-diff.sh feature/integration-<feature> main
```
...
**Language-specific rules:** See `~/.copilot/rules/coding-standards-{language}.md`
...
```bash
~/.copilot/scripts/workflow-state.sh set docs/workflow-state/<feature>.state.json '.phase = "synthesize"'
```
...
2. Output: "Quality review passed. Continuing to synthesis..."
3. The synthesis skill will auto-activate

### If NEEDS_FIXES:
1. Update state with failed issues
2. Output: "Quality review found [N] HIGH priority issues. Continuing to fixes..."
3. The delegation skill will auto-activate in fix mode

### If BLOCKED:
1. Update state: `.phase = "blocked"`
2. Output: "Quality review blocked: [issue]. Returning to design..."
3. The brainstorming skill will auto-activate for redesign
