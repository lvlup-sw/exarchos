// Unit tests for eslint-rules/comment-content.js.
//
...
test('Rule_MeasuredOffenderFixture_Reported', () => {
  ...
  const fixture = path.join(REPO_ROOT, 'scripts/__fixtures__/comment-hygiene/offenders.ts');
...
test('Rule_PermittedFixture_NotReported', () => {
  const fixture = path.join(REPO_ROOT, 'scripts/__fixtures__/comment-hygiene/permitted.ts');
  const messages = lint(readFileSync(fixture, 'utf8'), fixture);
